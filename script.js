$(function () {
  ...
  function notifications_list_add(msg) {
    ...
    d.textContent =
      local_date.toLocaleDateString() + " " + local_date.toLocaleTimeString();
    ...
    if (msg.type == 0) {
      ...
      a.setAttribute(
        "href",
        "viewtopic.php?pid=" + parseInt(msg.extra) + "#p" + parseInt(msg.extra)
      );
  ...
  function notifications_list_mark_seen(id) {
    try {
      var li = $("#notifications-list")
        .find("a[data-notif-id=" + id.toString() + "]")
        .parent()
        .parent();
